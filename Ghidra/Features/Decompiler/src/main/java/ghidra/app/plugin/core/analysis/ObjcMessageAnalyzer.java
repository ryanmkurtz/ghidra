...
public class ObjcMessageAnalyzer extends AbstractAnalyzer {
	...
	private record Message(String receiver, String selector, PcodeOpAST op, int varargParamIndex,
			Address addr) {}
	...
	public void optionsChanged(Options options, Program program) {
		useCallOverrides = options.getBoolean(OPTION_NAME_CALL_OVERRIDE_REFS, true);
	}
	...
	private void fixMsgSendSignatures(Program program, TaskMonitor monitor, MessageLog log)
		...
		for (Function func : program.getFunctionManager().getFunctions(program.getMemory(), true)) {
			...
			try {
				...
				switch (name) {
					case "_objc_msgSend":
						params.add(new ParameterImpl("self", id, program));
						params.add(new ParameterImpl("op", sel, program));
						break;
					case "_objc_msgSend_fixup":
						params.add(new ParameterImpl("self", id, program));
						params.add(new ParameterImpl("message_ref", messageRefPtr, program));
						break;
					...
					case "_objc_msgSendSuper2":
						params.add(new ParameterImpl("super", objcSuperPtr, program));
						params.add(new ParameterImpl("op", sel, program));
						break;
	...
	private List<Message> findMessages(Program program, HighFunction highFunction,
		...
		for (PcodeOpAST op : CollectionUtils.asIterable(highFunction.getPcodeOps())) {
			...
			if (ObjectUtils.allNotNull(receiver, selector)) {
				messages.add(new Message(receiver, selector, op, 3 + stretParamShift, callTarget));
			}
		}
		return messages;
	}
	...
	private void fixMsgSendCalls(Program program, HighFunction highFunction,
		...
		for (Message msg : messages) {
			...
			setCommentAndReference(program, msg, parameters);
		}
	}
	
	private void updateExternalBlock(Program program, Message msg, MessageLog log) {
		FunctionManager funcMgr = program.getFunctionManager();
		SymbolTable symbolTable = program.getSymbolTable();
		String currentClassName = msg.receiver;
		String currentMethodName = msg.selector;

		List<Symbol> objcClassSymbols =
			symbolTable.getGlobalSymbols(ObjcUtils.OBJC_CLASS_SYMBOL_PREFIX + currentClassName);
		if (objcClassSymbols.isEmpty()) {
			// TODO: 
			log.appendMsg("Failed to find %s symbol for %s"
					.formatted(ObjcUtils.OBJC_CLASS_SYMBOL_PREFIX, msg));
			return;
		}

		Symbol objcClassSymbol = objcClassSymbols.getFirst();
		if (!program.getMemory().getBlock(objcClassSymbol.getAddress()).isExternalBlock()) {
			return;
		}

		int offset = classExternalSymbolOffset.getOrDefault(currentClassName, 1);
		if (offset >= ObjcUtils.OBJC_CLASS_SYMBOL_EXTERNAL_CHUNK_SIZE) {
		...
		try {
			...
			Address funcAddr = objcClassSymbol.getAddress().add(offset);
...
//		}
		...
		if (functionSymbols.size() == 1) {
			...
			// TODO: When alter the reference to an _objc_msgSendSuper2(), we should really do a
			// HighFunctionDBUtil.writeOverride(), changing the signature (objc_super*, SEL*).
			// Also, changing the type of the local that gets passed in as the first parameter to
			// a objc_super really cleans up the decompilation.
		}
	}
