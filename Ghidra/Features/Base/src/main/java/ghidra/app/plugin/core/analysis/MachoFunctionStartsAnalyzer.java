	...
	private void analyzeDyldCacheFunctionStarts(Program program, List<ByteProvider> providers,
		...
		try (SplitDyldCache splitDyldCache =
			...
			for (DyldCacheImageRecord imageRecord : imageRecords) {
				...
				DyldCacheHeader header = headers.get(imageRecord.splitCacheIndex());
				ByteProvider provider = providers.get(imageRecord.splitCacheIndex());
				monitor.checkCancelled();
				monitor.setMessage("Analyzing function starts for " + name + "...");
				monitor.incrementProgress(1);

				// Parse Mach-O header
				MachHeader machoHeader = new MachHeader(provider,
					imageRecord.image().getAddress() - header.getBaseAddress(), false);
