...
public class Objc2ProtocolList implements StructConverter {
	...
	public Objc2ProtocolList(ObjcState state, BinaryReader reader) throws IOException {
		this._state = state;
		...
		for (long i = 0 ; i < count ; ++i) {
			long protocolIndex = ObjcUtils.readNextIndex(reader, state.is32bit);
			long originalIndex = reader.getPointerIndex();
			reader.setPointerIndex(protocolIndex);
			protocols.add( new Objc2Protocol(state, reader) );
	...
	public void applyTo(Namespace namespace) throws Exception {
		...
		try {
			Namespace protocolListNamespace = ObjcUtils.createNamespace(_state.program, Objc1Constants.NAMESPACE, Objc2ProtocolList.NAME);
			ObjcUtils.createSymbol(_state.program, protocolListNamespace, namespace.getName(), address);
		}
		catch (Exception e) {}
