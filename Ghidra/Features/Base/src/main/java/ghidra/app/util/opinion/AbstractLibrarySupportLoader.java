...
public abstract class AbstractLibrarySupportLoader extends AbstractProgramLoader {
	...
	protected List<Loaded<Program>> loadProgram(ImporterSettings settings)
		...
		try {
			...
			if (!shouldLoadOnlyLibraries(settings)) {
				...
				String projectPath = LoaderUtils.createProjectPath(settings.projectRootPath(),
					settings.rootRelativeFolderPath(), settings.name());
	...
	protected DomainFolder getLinkSearchFolder(Program program, ImporterSettings settings) {
		...
		String projectFolderPath = LoaderUtils.createProjectPath(settings.projectRootPath(),
			settings.rootRelativeFolderPath());
	...
	protected List<DomainFolder> getLibrarySearchFolders(List<Loaded<Program>> loadedPrograms,
		...
		try {
			...
			DomainFolder linkSearchFolder =
				getLinkSearchFolder(firstProgram, settings);
		...
		try {
			...
			for (GFile candidateLibraryFile : candidateLibraryFiles) {
				...
				try (ByteProvider provider = createLibraryByteProvider(candidateLibraryFsrl,
					...
					String rootRelativeFolderPath = settings.mirrorFsLayout() || isAbsolute
							? FilenameUtils.getFullPath(candidateLibraryFsrl.getPath())
							: null;
					ImporterSettings librarySettings =
						new ImporterSettings(provider, simpleLibraryName, settings.project(),
							libraryDestFolderPath, rootRelativeFolderPath,
							settings.mirrorFsLayout(), libLoadSpec, options, consumer, log,
							monitor);
	...
	protected DomainFile findLibraryInProject(String library, DomainFolder rootSearchFolder,
		...
		if (isMirroredLayout(settings)) {
			...
			for (LibrarySearchPath searchPath : searchPaths) {
				...
				if (searchPath.relativeFsPath() != null) {
					fsPath = FSUtilities.appendPath(fsPath, searchPath.relativeFsPath());
				}
				String projectPath =
					LoaderUtils.createProjectPath(rootSearchFolder.getPathname(), fsPath, library);
		...
		String projectPath = LoaderUtils.createProjectPath(rootSearchFolder.getPathname(), library);
	 ...
	 */
	private Program loadLibrary(String libraryName, String libraryPath, FSRL libraryFsrl, List<String> libraryNameList,
			ImporterSettings settings) throws CancelledException, IOException {

		try (ByteProvider provider = createLibraryByteProvider(libraryFsrl, settings.loadSpec(),
			...
			if (libLoadSpec == null) {
				settings.log()
						.appendMsg(
							"Skipping library which is the wrong architecture: " + libraryFsrl);
				return null;
			}
			
			ImporterSettings librarySettings =
				new ImporterSettings(provider, libraryName, settings.project(),
					settings.projectRootPath(), libraryPath, settings.mirrorFsLayout(), libLoadSpec,
					settings.options(), settings.consumer(), settings.log(), settings.monitor());
			return doLoad(libraryNameList, librarySettings);
		}
	}
