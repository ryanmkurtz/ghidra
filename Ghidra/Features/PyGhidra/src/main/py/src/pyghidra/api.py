import contextlib
from typing import Union, TYPE_CHECKING, Tuple, Callable, Any, BinaryIO
...
def walk_filesystem(
        fs: "GFileSystem",
        callback: Callable[["GFile"], None]
    ):
    """
    Walks the the given GFileSystem, calling the provided function when each GFile is encountered.

    :param fs: The GFileSystem to walk.
    :param callback: The callback to process each GFile.
    :param start: An optional starting project folder path.
    :raises FileNotFoundError: If the starting folder is not found in the project.
    """
    from ghidra.formats.gfilesystem import FSUtilities
    ...
    for gfile in FSUtilities.listFileSystem(fs, None, None, TaskMonitor.DUMMY):
        callback(gfile)
