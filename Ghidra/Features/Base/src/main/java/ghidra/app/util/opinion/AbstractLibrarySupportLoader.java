...
public abstract class AbstractLibrarySupportLoader extends AbstractProgramLoader {
	...
	private List<Loaded<Program>> loadLibraries(ByteProvider provider, Program program,
		...
		Map<Integer, List<GFile>> libraryListingCache = new HashMap<>();
	...
	private Loaded<Program> loadLibraryFromSearchPaths(String libraryName,
			...
			Map<Integer, List<GFile>> libraryListingCache, LoadSpec desiredLoadSpec,
	...
	private List<FSRL> findLibrary(Path libraryPath, List<FileSystemSearchPath> fsSearchPaths,
			Map<Integer, List<GFile>> libraryListingCache, MessageLog log, TaskMonitor monitor)
	...
	protected FSRL resolveLibraryFile(GFileSystem fs, Path libraryParentPath, String libraryName,
			Map<Integer, List<GFile>> libraryListingCache) throws IOException {
		...
		if (libraryParentDir != null) {
			...
			for (GFile file : getCachedLibraryListing(fs, libraryParentDir, libraryListingCache)) {
				if (file.isDirectory()) {
					continue;
				}
	...
	protected List<GFile> getCachedLibraryListing(GFileSystem fs, GFile libraryParentDir,
			Map<Integer, List<GFile>> libraryListingCache) throws IOException {
		int key = Objects.hash(fs, libraryParentDir);
		List<GFile> listing = libraryListingCache.get(key);
		if (listing == null) {
			listing = fs.getListing(libraryParentDir);
			libraryListingCache.put(key, listing);
		}
		return listing;
	}
