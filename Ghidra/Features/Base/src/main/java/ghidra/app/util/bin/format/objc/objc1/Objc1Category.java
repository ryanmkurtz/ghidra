...
public class Objc1Category implements StructConverter {
	...
	public Objc1Category(ObjcState state, BinaryReader reader) throws IOException {
		this._state = state;
		this._index = reader.getPointerIndex();

		category_name    = reader.readAsciiString( reader.readNextInt() );
		class_name       = reader.readAsciiString( reader.readNextInt() );
		instance_methods = new Objc1MethodList(state, reader.clone(reader.readNextInt()), ObjcMethodType.INSTANCE);
		class_methods    = new Objc1MethodList(state, reader.clone(reader.readNextInt()), ObjcMethodType.CLASS);
	...
	public DataType toDataType() throws DuplicateNameException, IOException {
		...
		struct.add(PointerDataType.getPointer(Objc1MethodList.toGenericDataType(_state), _state.pointerSize), "instance_methods", null);
		struct.add(PointerDataType.getPointer(Objc1MethodList.toGenericDataType(_state), _state.pointerSize), "class_methods", null);
	...
	public void applyTo() throws Exception {
		...
		Namespace namespace = ObjcUtils.createNamespace(_state.program, Objc1Constants.NAMESPACE, "Categories", class_name+'('+category_name+')');
