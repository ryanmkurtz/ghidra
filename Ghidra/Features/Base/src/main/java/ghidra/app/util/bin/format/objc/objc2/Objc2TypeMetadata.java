...
public class Objc2TypeMetadata extends AbstractObjcTypeMetadata {
	...
	public Objc2TypeMetadata(Program program, TaskMonitor monitor, MessageLog log)
		...
		super(new ObjcState(program, monitor, Objc2Constants.CATEGORY_PATH), program, monitor, log);
		parse();
	}
	...
	private void parseImageInfo(String section, BinaryReader reader,
		...
		for (MemoryBlock block : objcBlockMap.getOrDefault(section, List.of())) {
			...
			imageInfos.add(new Objc2ImageInfo(state, reader));
		}
	}
			...
			for (MemoryBlock block : objcBlockMap.getOrDefault(section, List.of())) {
				...
				for (int i = 0; i < count; ++i) {
					...
					long categoryAddress =
						state.is32bit ? reader.readUnsignedInt(address.getOffset())
								: reader.readLong(address.getOffset());
					reader.setPointerIndex(categoryAddress);
					categories.add(new Objc2Category(state, reader));
					refs.add(address);
					address = address.add(state.pointerSize);
				}
			}
		...
		try {
			for (MemoryBlock block : objcBlockMap.getOrDefault(section, List.of())) {
				...
				for (int i = 0; i < count; ++i) {
					...
					classes.add(new Objc2Class(state, reader));
					refs.add(address);
					address = address.add(state.pointerSize);
				}
			}
		...
		try {
			for (MemoryBlock block : objcBlockMap.getOrDefault(section, List.of())) {
				...
				for (int i = 0; i < count; ++i) {
					...
					Objc2Protocol protocol = new Objc2Protocol(state, reader);
	...
	private void parseMessageReferences(String section, BinaryReader reader,
		...
		try {
			for (MemoryBlock block : objcBlockMap.getOrDefault(section, List.of())) {
				long count = block.getSize() / Objc2MessageReference.SIZEOF(state);
				monitor.initialize((int) count);
				Address address = block.getStart();
				for (int i = 0; i < count; ++i) {
					monitor.increment();
					reader.setPointerIndex(address.getOffset());
					messageRefs.add(new Objc2MessageReference(state, reader));
					address = address.add(Objc2MessageReference.SIZEOF(state));
				}
			}
		}
		catch (IOException e) {
	...
	public void applyTo() {
		...
		ObjcUtils.createMethods(state);
		ObjcUtils.createInstanceVariablesC2_OBJC2(state);
		ObjcUtils.fixupReferences(Objc2Constants.getObjectiveC2SectionNames(), state);

		ObjcUtils.setBlocksReadOnly(state.program.getMemory(), List.of(
