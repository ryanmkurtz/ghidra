...
public class ImporterDialog extends DialogComponentProvider {
	...
	protected void okCallback() {
		...
		String programPathPrefix = lastIndexOf != -1 ? programPath.substring(0, lastIndexOf) : "";
		final String projectFolderPath;
		final String mirrorFolderPath;
		if (mirrorFsCheckBox.isSelected()) {
			mirrorFolderPath = destinationFolder.getPathname();
			projectFolderPath = programPathPrefix;
		}
		else {
			mirrorFolderPath = null;
			projectFolderPath =
				FSUtilities.appendPath(destinationFolder.getPathname(), programPathPrefix);
		}
		...
		new TaskBuilder("Import File", monitor -> {
			ImporterUtilities.importSingleFile(tool, programManager, fsrl, projectFolderPath,
				mirrorFolderPath, loadSpec, programName, options, monitor);
		})
		.setLaunchDelay(0)
		.launchNonModal();
		//@formatter:on
