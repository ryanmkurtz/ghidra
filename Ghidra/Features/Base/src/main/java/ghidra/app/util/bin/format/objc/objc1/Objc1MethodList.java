...
public class Objc1MethodList extends ObjcMethodList {
	...
	Objc1MethodList(ObjcState state, BinaryReader reader, ObjcMethodType methodType) throws IOException {
		...
		obsolete = new Objc1MethodList(state, reader.clone(reader.readNextInt()), methodType );

		method_count = reader.readNextInt();

		for (int i = 0 ; i < method_count ; ++i) {
			methods.add(new Objc1Method(state, reader, methodType));
		}
	}
