...
public class Objc1MetaClass implements StructConverter {
	...
	Objc1MetaClass(ObjcState state, BinaryReader reader) throws IOException {
		this._state = state;
		this._index = reader.getPointerIndex();

		isa                = ObjcUtils.dereferenceAsciiString(reader, state.is32bit);
		...
		variable_list      = new Objc1InstanceVariableList(state, reader.clone(reader.readNextInt()));
		method_list        = new Objc1MethodList(state, reader.clone(reader.readNextInt()), ObjcMethodType.INSTANCE);
	...
	public DataType toDataType() throws DuplicateNameException, IOException {
		...
		struct.add(PointerDataType.getPointer(Objc1InstanceVariableList.toGenericDataType(), _state.pointerSize), "instance_vars", null);
		struct.add(PointerDataType.getPointer(Objc1MethodList.toGenericDataType(_state), _state.pointerSize), "method_lists", null);
		struct.add(DWORD, "cache", null);
		struct.add(PointerDataType.getPointer(Objc1ProtocolList.toGenericDataType(_state), _state.pointerSize), "protocols", null);
	...
	public void applyTo() throws Exception {
		...
		Namespace namespace = ObjcUtils.createNamespace(_state.program, Objc1Constants.NAMESPACE, "Meta-classes", name);
