...
Python library that provides direct access to the Ghidra API within a native CPython interpreter 
...
plugin to allow the use of CPython from the Ghidra GUI.
...
GhidraScript's written in native CPython, as well as interact with the Ghidra GUI through a built-in
REPL. To launch launch Ghidra in PyGhidra-mode, see Ghidra's latest
...
However, it is possible (and encouraged!) to use PyGhidra as a standalone Python library for usage 
in reverse engineering workflows where Ghidra may be one of many components involved. The following 
instructions focus on this type of usage.
...
## API
PyGhidra inherits an initial API from the original "Pyhidra" project that provides an excellent 
starting point for interacting with a Ghidra installation. __NOTE:__ These functions are subject to
change in the future as more thought and feedback is collected on PyGhidra's role in the greater
Ghidra ecosystem:
