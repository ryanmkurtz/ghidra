...
public class Objc2Property implements StructConverter {
	private ObjcState _state;

	private String name;
	private String attributes;

	public Objc2Property(ObjcState state, BinaryReader reader) throws IOException {
		this._state = state;

		long nameIndex = ObjcUtils.readNextIndex(reader, state.is32bit);
		name  = reader.readAsciiString(nameIndex);

		long attributesIndex = ObjcUtils.readNextIndex(reader, state.is32bit);
