...
public class Objc1InstanceVariable implements StructConverter {
	...
	Objc1InstanceVariable(ObjcState state, BinaryReader reader) throws IOException {
		this._state = state;
		this._index = reader.getPointerIndex();

		name = ObjcUtils.dereferenceAsciiString(reader, state.is32bit);
		type = ObjcUtils.dereferenceAsciiString(reader, state.is32bit);
