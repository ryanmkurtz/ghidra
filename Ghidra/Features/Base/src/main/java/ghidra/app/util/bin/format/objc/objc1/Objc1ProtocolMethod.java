...
public class Objc1ProtocolMethod implements StructConverter {
	...
	Objc1ProtocolMethod(ObjcState state, BinaryReader reader, ObjcMethodType methodType) throws IOException {
		this._state = state;
		this._methodType = methodType;

		name  = ObjcUtils.dereferenceAsciiString(reader, state.is32bit);
		types = ObjcUtils.dereferenceAsciiString(reader, state.is32bit);
	}
