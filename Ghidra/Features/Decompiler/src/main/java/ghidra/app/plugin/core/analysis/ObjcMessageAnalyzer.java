...
public class ObjcMessageAnalyzer extends AbstractAnalyzer {
	...
	private boolean logMessageFailures = true;
	...
	public void registerOptions(Options options, Program program) {
		...
		options.registerOption(OPTION_NAME_LOG_MESSAGE_FAILURES, OptionType.BOOLEAN_TYPE,
			useCallOverrides, null, OPTION_DESCRIPTION_LOG_MESSAGE_FAILURES);
	}

	@Override
	public void optionsChanged(Options options, Program program) {
		useCallOverrides = options.getBoolean(OPTION_NAME_CALL_OVERRIDE_REFS, true);
		logMessageFailures = options.getBoolean(OPTION_NAME_LOG_MESSAGE_FAILURES, true);
	}
	...
	private List<Message> findMessages(Program program, HighFunction highFunction,
		...
		for (PcodeOpAST op : CollectionUtils.asIterable(highFunction.getPcodeOps())) {
			...
			if (!Strings.CS.equals(mnemonic, "CALL") && !Strings.CS.equals(mnemonic, "CALLIND")) {
				continue;
			}
